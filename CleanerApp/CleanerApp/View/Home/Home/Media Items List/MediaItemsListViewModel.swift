...
class MediaItemsListViewModel: ObservableObject {
    ...
    func btnSelectAllAction() {
        withAnimation {
            if arrSelectedItems.count != arrItems.count {
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    let viewModel = StillPhotoPreviewModel(arrImageURLs: arrImageURLs)
                    viewModel.onSaveAction = { [weak self] imageURL in
                        guard let self = self else { return }
                        self.convertStill(imageURL: imageURL)
                    }
                    
                    NavigationManager.shared.push(to: .stillPhotoPreviewView(destination: StillPhotoPreviewDestination(viewModel: viewModel)))
                }
            } catch {
                await MainActor.run {
                    CNLoader.dismiss()
    ...
    func convertStill(imageURL: URL) {
        NavigationManager.shared.pop()
